#!/bin/bash
# ============================================================
# 🔩 SukiSU-Ultra Manual Hook Inserter (awk-based)
# Applies manual hooks for non-GKI kernels
# ============================================================
set -euo pipefail

OPTIONAL=false
if [ "${1:-}" = "--optional" ]; then
  OPTIONAL=true
fi

HOOKS_APPLIED=0
HOOKS_FAILED=0

# ── Helper functions ──────────────────────────────────────
apply_hook() {
  local FILE="$1"
  local HOOK_NAME="$2"
  local PATTERN="$3"
  local INSERT_CODE="$4"
  local POSITION="${5:-before}"  # before or after

  if [ ! -f "$FILE" ]; then
    echo "  ❌ File not found: $FILE"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
    return 1
  fi

  # Check if already applied
  if grep -q "CONFIG_KSU.*CONFIG_KSU_MANUAL_HOOK" "$FILE" 2>/dev/null; then
    echo "  ℹ️ $HOOK_NAME already applied, skipping"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
    return 0
  fi

  # Find pattern line
  LINE_NUM=$(grep -n "$PATTERN" "$FILE" | head -1 | cut -d: -f1)
  if [ -z "$LINE_NUM" ]; then
    echo "  ❌ Pattern not found in $FILE: $PATTERN"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
    return 1
  fi

  echo "  📍 Found pattern at line $LINE_NUM"

  if [ "$POSITION" = "before" ]; then
    sed -i "${LINE_NUM}i\\${INSERT_CODE}" "$FILE"
  else
    sed -i "${LINE_NUM}a\\${INSERT_CODE}" "$FILE"
  fi

  # Verify
  if grep -q "CONFIG_KSU.*CONFIG_KSU_MANUAL_HOOK\|ksu_handle" "$FILE" 2>/dev/null; then
    echo "  ✅ $HOOK_NAME applied successfully"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ $HOOK_NAME verification failed"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
}

echo "════════════════════════════════════════════════════"
echo "🔩 SukiSU-Ultra Manual Hook Inserter"
echo "════════════════════════════════════════════════════"
echo ""

# ── Hook 1: fs/exec.c ────────────────────────────────────
echo "📌 Hook 1/5: fs/exec.c (do_execve)"
if [ -f fs/exec.c ] && ! grep -q "ksu_handle_execveat" fs/exec.c 2>/dev/null; then
  # Insert extern declarations before do_execve
  awk '
  /^int do_execve\(/ && !done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "extern bool ksu_execveat_hook __read_mostly;"
    print "extern int ksu_handle_execveat(int *fd, struct filename **filename_ptr, void *argv,"
    print "\t\t\tvoid *envp, int *flags);"
    print "extern int ksu_handle_execveat_sucompat(int *fd, struct filename **filename_ptr,"
    print "\t\t\tvoid *argv, void *envp, int *flags);"
    print "#endif"
    print ""
    done=1
  }
  { print }
  ' fs/exec.c > fs/exec.c.tmp && mv fs/exec.c.tmp fs/exec.c

  # Insert hook call inside do_execve (after envp initialization)
  awk '
  /struct user_arg_ptr envp = \{/ && !hook1_done {
    print
    getline
    print
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "\tif (unlikely(ksu_execveat_hook))"
    print "\t\tksu_handle_execveat(&AT_FDCWD, &filename, &argv, &envp, 0);"
    print "\telse"
    print "\t\tksu_handle_execveat_sucompat(&AT_FDCWD, &filename, &argv, &envp, 0);"
    print "#endif"
    hook1_done=1
    next
  }
  { print }
  ' fs/exec.c > fs/exec.c.tmp && mv fs/exec.c.tmp fs/exec.c

  # Insert sucompat hook in compat_do_execve
  awk '
  /^int compat_do_execve\(/ { in_compat=1 }
  in_compat && /return do_execveat_common/ && !hook2_done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "\tif (unlikely(!ksu_execveat_hook))"
    print "\t\tksu_handle_execveat_sucompat(&AT_FDCWD, &filename, &argv, &envp, 0);"
    print "#endif"
    hook2_done=1
  }
  { print }
  ' fs/exec.c > fs/exec.c.tmp && mv fs/exec.c.tmp fs/exec.c

  if grep -q "ksu_handle_execveat" fs/exec.c; then
    echo "  ✅ fs/exec.c hooks applied"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/exec.c hook failed"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
else
  if grep -q "ksu_handle_execveat" fs/exec.c 2>/dev/null; then
    echo "  ℹ️ Already applied, skipping"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/exec.c not found"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
fi

# ── Hook 2: fs/open.c ────────────────────────────────────
echo ""
echo "📌 Hook 2/5: fs/open.c (faccessat)"
if [ -f fs/open.c ] && ! grep -q "ksu_handle_faccessat" fs/open.c 2>/dev/null; then
  awk '
  /SYSCALL_DEFINE3\(faccessat/ && !done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "extern int ksu_handle_faccessat(int *dfd, const char __user **filename_user, int *mode,"
    print "\t\t\tint *flags);"
    print "#endif"
    print ""
    done=1
  }
  { print }
  ' fs/open.c > fs/open.c.tmp && mv fs/open.c.tmp fs/open.c

  awk '
  /SYSCALL_DEFINE3\(faccessat/ { in_func=1 }
  in_func && /return do_faccessat/ && !hook_done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "\tksu_handle_faccessat(&dfd, &filename, &mode, NULL);"
    print "#endif"
    hook_done=1
  }
  { print }
  ' fs/open.c > fs/open.c.tmp && mv fs/open.c.tmp fs/open.c

  if grep -q "ksu_handle_faccessat" fs/open.c; then
    echo "  ✅ fs/open.c hook applied"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/open.c hook failed"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
else
  if grep -q "ksu_handle_faccessat" fs/open.c 2>/dev/null; then
    echo "  ℹ️ Already applied, skipping"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/open.c not found"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
fi

# ── Hook 3: fs/read_write.c ──────────────────────────────
echo ""
echo "📌 Hook 3/5: fs/read_write.c (sys_read)"
if [ -f fs/read_write.c ] && ! grep -q "ksu_handle_sys_read" fs/read_write.c 2>/dev/null; then
  awk '
  /SYSCALL_DEFINE3\(read,/ && !done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "extern bool ksu_vfs_read_hook __read_mostly;"
    print "extern int ksu_handle_sys_read(unsigned int fd, char __user **buf, size_t *count);"
    print "#endif"
    print ""
    done=1
  }
  { print }
  ' fs/read_write.c > fs/read_write.c.tmp && mv fs/read_write.c.tmp fs/read_write.c

  awk '
  /SYSCALL_DEFINE3\(read,/ { in_func=1 }
  in_func && /return ksys_read/ && !hook_done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "\tif (unlikely(ksu_vfs_read_hook))"
    print "\t\tksu_handle_sys_read(fd, &buf, &count);"
    print "#endif"
    hook_done=1
  }
  { print }
  ' fs/read_write.c > fs/read_write.c.tmp && mv fs/read_write.c.tmp fs/read_write.c

  if grep -q "ksu_handle_sys_read" fs/read_write.c; then
    echo "  ✅ fs/read_write.c hook applied"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/read_write.c hook failed"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
else
  if grep -q "ksu_handle_sys_read" fs/read_write.c 2>/dev/null; then
    echo "  ℹ️ Already applied, skipping"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/read_write.c not found"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
fi

# ── Hook 4: fs/stat.c ────────────────────────────────────
echo ""
echo "📌 Hook 4/5: fs/stat.c (newfstatat)"
if [ -f fs/stat.c ] && ! grep -q "ksu_handle_stat" fs/stat.c 2>/dev/null; then
  awk '
  /SYSCALL_DEFINE4\(newfstatat/ && !done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "extern int ksu_handle_stat(int *dfd, const char __user **filename_user, int *flags);"
    print "#endif"
    print ""
    done=1
  }
  { print }
  ' fs/stat.c > fs/stat.c.tmp && mv fs/stat.c.tmp fs/stat.c

  awk '
  /SYSCALL_DEFINE4\(newfstatat/ { in_func=1 }
  in_func && /error = vfs_fstatat/ && !hook_done {
    print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
    print "\tksu_handle_stat(&dfd, &filename, &flag);"
    print "#endif"
    hook_done=1
  }
  { print }
  ' fs/stat.c > fs/stat.c.tmp && mv fs/stat.c.tmp fs/stat.c

  if grep -q "ksu_handle_stat" fs/stat.c; then
    echo "  ✅ fs/stat.c hook applied"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/stat.c hook failed"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
else
  if grep -q "ksu_handle_stat" fs/stat.c 2>/dev/null; then
    echo "  ℹ️ Already applied, skipping"
    HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
  else
    echo "  ❌ fs/stat.c not found"
    HOOKS_FAILED=$((HOOKS_FAILED + 1))
  fi
fi

# ── Hook 5 (optional): drivers/input/input.c ─────────────
echo ""
if [ "$OPTIONAL" = true ]; then
  echo "📌 Hook 5/5: drivers/input/input.c (Safe Mode)"
  if [ -f drivers/input/input.c ] && ! grep -q "ksu_handle_input_handle_event" drivers/input/input.c 2>/dev/null; then
    awk '
    /static void input_handle_event\(/ && !done {
      print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
      print "extern void ksu_handle_input_handle_event(unsigned int *type, unsigned int *code, int *value);"
      print "#endif"
      print ""
      done=1
    }
    { print }
    ' drivers/input/input.c > drivers/input/input.c.tmp && mv drivers/input/input.c.tmp drivers/input/input.c

    awk '
    /int disposition = input_get_disposition/ && !hook_done {
      print
      print "#if defined(CONFIG_KSU) && defined(CONFIG_KSU_MANUAL_HOOK)"
      print "\tksu_handle_input_handle_event(&type, &code, &value);"
      print "#endif"
      hook_done=1
      next
    }
    { print }
    ' drivers/input/input.c > drivers/input/input.c.tmp && mv drivers/input/input.c.tmp drivers/input/input.c

    if grep -q "ksu_handle_input_handle_event" drivers/input/input.c; then
      echo "  ✅ input.c hook applied"
      HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
    else
      echo "  ❌ input.c hook failed"
      HOOKS_FAILED=$((HOOKS_FAILED + 1))
    fi
  else
    if grep -q "ksu_handle_input_handle_event" drivers/input/input.c 2>/dev/null; then
      echo "  ℹ️ Already applied, skipping"
      HOOKS_APPLIED=$((HOOKS_APPLIED + 1))
    else
      echo "  ❌ drivers/input/input.c not found"
      HOOKS_FAILED=$((HOOKS_FAILED + 1))
    fi
  fi
else
  echo "📌 Hook 5/5: drivers/input/input.c (Safe Mode) — SKIPPED (optional)"
fi

# ── Summary ───────────────────────────────────────────────
echo ""
echo "════════════════════════════════════════════════════"
echo "📊 Hook Summary"
echo "════════════════════════════════════════════════════"
echo "  ✅ Applied: $HOOKS_APPLIED"
echo "  ❌ Failed:  $HOOKS_FAILED"
echo ""

if [ $HOOKS_FAILED -gt 0 ]; then
  echo "⚠️ Some hooks failed! Build may not work correctly."
  exit 1
else
  echo "✅ All hooks applied successfully!"
fi
